...
major, minor, patch = (int(component) for component in last_release_version[1:].split("."))

# Get prefixes (see CONTRIBUTING.md) of commits since last release
commits_since_last_release = subprocess.check_output(
    ["git", "log", f"{last_release_version}..HEAD", "--pretty=format:%s"], encoding="utf-8"
).splitlines()
prefixes = set()
...
if "breaking" in prefixes:
    ...
    raise ValueError("No commits since last release with valid prefix (see CONTRIBUTING.md).")
