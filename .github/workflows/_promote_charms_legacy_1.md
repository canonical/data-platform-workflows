Workflow file: [_promote_charms_legacy_1.yaml](_promote_charms_legacy_1.yaml)

> [!WARNING]
> This workflow is **deprecated**. For new tracks, use [_promote_charms.yaml](_promote_charms.md) instead.
>
> Subject to **breaking changes on patch release**. `_promote_charms_legacy_1.yaml` is experimental & not part of the public interface.
...
jobs:
  promote:
    name: Promote charms
    uses: canonical/data-platform-workflows/.github/workflows/_promote_charms_legacy_1.yaml@v0.0.0
    with:
      track: 'latest'
